...
3.  Unload the data from the `REGION` table but only the data from columns
...
4.  Using the second session review the file that was created,
...
3.  Using the second session at the OS command line, use the nzload
	...
	The following command will load data from the `et1_region_flat_file` into the `REGION`
...
13. Verify the new row has been loaded into the table:
	...
	The row with `***` denotes the new row that was added to the table,
