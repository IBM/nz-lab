...
3.  Prepare for this lab by running the setup script. To do this use the
	...
	There may be error messages at the beginning of the output since the
	script tries to clean up existing databases and users.

4.  Switch to the lab directory `~/labs/optimizationObjects`. To do this
...
4.  Now let's have a look at the cost of this query. To get the
	projected cost from the Optimizer we use the following `EXPLAIN VERBOSE` command:
...
2.  The following command returns the number of returned shipments vs.
	...
	You can see that on the 15th June of 1995 there have been 176
...
5.  Now we will have a look at the zone map values for the `L_SHIPDATE`
	...
	This means that our query which has a `WHERE` condition on the 15th
...
overhead during `INSERT`s, `UPDATE`s and `DELETE`s, requires additional disc
...
their main performance improvements on one column. Also, `INSERT`s to the
...
3.  We have supplied a script for the creation of the `ORDERS_CBT`
...
7.  Enter `:wq!` In the command line and press Enter to save and
...
Table can become partially unordered due to `INSERT`s, `UPDATE`s and
`DELETE`s. A threshold is defined for reorganization and the groom command
...
2.  Execute the following command to groom your cluster-based table:
	...
	!!! warning 
		This command requires a lot of RAM on
